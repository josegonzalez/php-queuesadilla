...
class PdoBackend extends Backend {
  ...
  public function release($item, $queue = null) {
    $sql = sprintf('UPDATE `%s` SET locked = 0 WHERE id = ?', $this->_settings['table']);
  ...
  public function delete($item) {
    $sql = sprintf('DELETE FROM `%s` WHERE id = ?', $this->_settings['table']);
    $sth = $this->_connection->prepare($sql);
    $sth->bindParam(1, $item['id'], PDO::PARAM_INT);
    $sth->execute();
    return $sth->rowCount() == 1;
  ...
  public function pop($queue = null) {
    ...
    $sql = sprintf('SELECT `id`, `data` FROM `%s` WHERE `queue` = ? and `locked` != 1 ORDER BY id asc LIMIT 1', $this->_settings['table']);
    $sth = $this->_connection->prepare($sql);
    ...
    $sql = sprintf('UPDATE `%s` SET locked = 1 WHERE id = ?', $this->_settings['table']);
    $sth = $this->_connection->prepare($sql);
  ...
  protected function _push($item, $queue = null) {
    ...
    $sql = sprintf('INSERT INTO `%s` (`data`, `queue`) VALUES (?, ?)', $this->_settings['table']);
    $sth = $this->_connection->prepare($sql);
    $sth->bindParam(1, json_encode($item), PDO::PARAM_STR);
    $sth->bindParam(2, $queue, PDO::PARAM_STR);
    $sth->execute();
    $this->_last_job_id = $this->_connection->lastInsertId();
  }
